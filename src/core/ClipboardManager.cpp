...
ClipboardManager::ClipboardManager(QObject *parent)
  ...
  qDebug() << "Creating ClipboardManager with parent:"
           << (parent ? "yes" : "no");
  ...
  if (m_clipboard) {
    ...
    qCritical() << "ClipboardManager created without valid clipboard - may not "
                   "function properly";
  }
...
bool ClipboardManager::isValidText(const QString &text) {
  ...
  if (text.length() > Constants::MAX_CLIPBOARD_TEXT_LENGTH) {
    qDebug() << "Text exceeds" << Constants::MAX_CLIPBOARD_TEXT_LENGTH
             << "limit - not valid";
    qDebug() << "ClipboardManager::isValidText() - EXIT (returning false)";
    return false;
  }
